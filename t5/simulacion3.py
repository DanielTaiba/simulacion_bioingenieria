...
class labRandom(lab):
    def __init__(self, M: int = 32, P: float = 0.15, R: int = 1, PM: float = 0.6, T1: int = 5, T2: int = 7,type_neighboor:str='Circle', num_simulacion: int = 0, path: str = './experimentos/') -> None:
        super().__init__(M, P, R, PM, T1, T2,type_neighboor ,num_simulacion, path)
...
if __name__=='__main__':
    ...
    for i in range(1,cantidad_simulaciones+1):
        ...
        simulacion=labParalelo(
            ...
            T1=3,
            T2=5,
            type_neighboor='Neumann', #opciones validas: Moore , Neumann , Circle (default) 
            #no cambiar
            num_simulacion=i,
            path=dir
        )
